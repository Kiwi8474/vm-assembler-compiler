# MX-26201 Technical Specification
...
## 2. Register File
The MX-26201 features 16 registers (r0 through r15)
...
| `0x0200 - 0x03FF` | **Boot Sector** | Sector 0 of disk gets loaded from BIOS | Initial boot code |
| `0x0400 - 0x7FFF` | **General RAM** | 31 KiB storage | Buffer for disk sectors and programs |
| `0x8000 - 0x8FA0` | **VRAM** | 4000 Bytes | 16 colors text-mode display buffer (80x25) |
| `0x8FA1 - 0xFFFB` | **General RAM** | ~30 KiB storage | Primary workspace and variable storage |
| `0xFFFB - 0xFFFF` | **MMIO Ports** | 5 Bytes | Hardware I/O (Mouse/Keyboard) |
...
| 0x6 | jgt | Jump to **RegC** if **RegA** > **RegB** |
| 0x7 | out | Write Data **RegB** to Port **RegA** |
| 0x8 | je | Jump to **RegC** if **RegA** == **RegB** |
| 0x9 | jne | Jump to **RegC** if **RegA** != **RegB** |
...
| 0xC | jlt | Jump to **RegC** if **RegA** < **RegB** |
| 0xD | jge | Jump to **RegC** if **RegA** >= **RegB**|
| 0xE | pop | Pop from stack to **RegA** |
| 0xF | push | Push **RegA** onto stack |

## 5. Hardware Interfacing (I/O)
### 5.1 Text Display (VRAM)
The display is a 16 color 80x25 text grid. Writing an ASCII value to `0x8000 - 0x8FA0` will render the corresponding character.

### 5.2 Input Ports (MMIO)
| Address | Description |
| :--- | :--- |
| `0xFFFB` | Generates a random byte when reading |
...
| :--- | :--- | :--- |
| `0x1` | DEBUG_CHAR | Prints **RegB** as ASCII character to stdout |
| `0x2` | DEBUG_HEX | Prints **RegB** as int and hex to stdout |
...
| `0x20` | VGA_CTRL | Sets the VGA mode (0: monochrome, 1: 16 colors) |
...
| `0x40` | WAIT | Pauses execution for X ms |
